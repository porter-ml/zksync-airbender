...
pub fn merge_rs2_and_memload_access<F: PrimeField, C: Circuit<F>>(
    ...
    let execute_mem_load = Boolean::and(&execute_mem_family, &should_write_mem_if_mem.toggle(), cs);

    assert_eq!(rs2_load_timestamp, rs2_mem_query.local_timestamp_in_cycle);
    assert_eq!(rs2_load_timestamp, mem_load_query.local_timestamp_in_cycle);
    ...
    let ShuffleRamQueryType::RegisterOnly { register_index } = rs2_mem_query.query_type else {
        panic!()
    };
    ...
    let ShuffleRamQueryType::RegisterOrRam {
        is_register: _,
        address: ram_query_address,
    } = mem_load_query.query_type
    else {
        panic!()
    };

    let addr_low = cs.choose(
        execute_mem_load,
        Num::Var(ram_query_address[0]),
        Num::Var(register_index),
    );
    let addr_high = cs.choose(
        execute_mem_load,
        Num::Var(ram_query_address[1]),
        Num::Constant(F::ZERO),
    );

    let query_type = ShuffleRamQueryType::RegisterOrRam {
        is_register,
        address: [addr_low.get_variable(), addr_high.get_variable()],
    };

    // NOTE on read timestamp - it's purely a witness variable, so we do not even need to choose it.
    // We still have to provide read value because we use it, but in general it can also be made pure witness
    // at the cost of additional rework of few selectors

    let mut query = mem_load_query;
    query.query_type = query_type;
    query.read_value = std::array::from_fn(|i| {
        cs.choose(
            execute_mem_load,
            Num::Var(query.read_value[i]),
            Num::Var(rs2_mem_query.read_value[i]),
        )
        .get_variable()
    });
    query.write_value = query.read_value;

    memory_queries.push(query);
}

pub(crate) fn merge_rs2_and_memload_access_optimized<F: PrimeField, C: Circuit<F>>(
    cs: &mut C,
    opcodes_are_in_rom: bool,
    flags_source: &BasicFlagsSource,
    ...
    let addr_low = cs.add_variable_from_constraint(
        (Term::from(1) - Term::from(execute_mem_load.get_variable().unwrap())) * rs2_mem_query.rs2
            + Term::from(execute_mem_load.get_variable().unwrap())
                * Term::from(ram_query_address[0]),
    );
    let addr_high = cs.add_variable_from_constraint(
        Term::from(execute_mem_load.get_variable().unwrap()) * Term::from(ram_query_address[1]),
    );
...
pub(crate) fn update_register_op_as_shuffle_ram_optimized<F: PrimeField, C: Circuit<F>>(
    cs: &mut C,
    local_timestamp_in_cycle: usize,
    rd_constraint: Constraint<F>,
    reg_value: Register<F>,
    execute_register_update: Constraint<F>,
    memory_store_query_to_merge: ShuffleRamMemQuery,
    execute_memory_store: Boolean,
) -> ShuffleRamMemQuery {
    assert_eq!(
    ...
    // it is boolean by construction

    // If we write to x0, then we will write 0,
    // so if we do NOT need to execute register update, OR if dst register is x0, then we must mask a value,
    // so the only case when we do not mask is if reg !=0 and we execute register update
    let do_not_need_masking = cs.add_variable_from_constraint(
        (Term::from(1) - Term::from(reg_is_zero.get_variable().unwrap()))
            * execute_register_update.clone(),
    );
    let reg_write_value = reg_value.mask(cs, Boolean::Is(do_not_need_masking));
    ...
    let addr_low = cs.add_variable_from_constraint(
        rd_constraint * execute_register_update // register
        + (Term::from(ram_query_address[0]) * Term::from(execute_memory_store)), // RAM
    );
    let addr_high = cs.add_variable_from_constraint(
        Term::from(ram_query_address[1]) * Term::from(execute_memory_store),
