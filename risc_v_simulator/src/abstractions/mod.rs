...
#[must_use]
#[inline(always)]
pub fn mem_read<M: MemorySource, TR: Tracer<C>, C: MachineConfig>(
    memory_source: &mut M,
    tracer: &mut TR,
    phys_address: u64,
    num_bytes: u32,
        ...
        let value = match (unalignment, num_bytes) {
            (0, 4) | (0, 2) | (2, 2) | (0, 1) | (1, 1) | (2, 1) | (3, 1) => {
                ...
                if access_type == AccessType::Instruction {
                    tracer.trace_opcode_read(aligned_address, value);
                } else {
                    tracer.trace_ram_read(aligned_address, value);
                }
...
#[inline(always)]
pub fn mem_write<M: MemorySource, TR: Tracer<C>, C: MachineConfig>(
    memory_source: &mut M,
    tracer: &mut TR,
    phys_address: u64,
    value: u32,
    ...
    if C::SUPPORT_LOAD_LESS_THAN_WORD {
        match (unalignment, num_bytes) {
            ...
            | a @ (3, 1) => {
                ...
                tracer.trace_ram_read_write(aligned_address, old_value, new_value);
                if trap.is_a_trap() {
                    return;
                }
            }
